#!/usr/bin/env python3
"""
Fix all JSONResponse calls in cameras.py
"""

def fix_cameras_jsonresponse():
    """Fix all JSONResponse calls in cameras.py"""
    ...
    # Fix all JSONResponse patterns
    import re
    
    # Pattern 1: JSONResponse with create_json_response
    content = re.sub(
        r'return JSONResponse\(\s*content=create_json_response\(([^)]+)\),\s*status_code=status\.HTTP_200_OK\s*\)',
        r'return create_json_response(\1)',
        content
    )
    
    # Pattern 2: JSONResponse with create_error_json_response
    content = re.sub(
    ...
    print("Fixed cameras.py JSONResponse issues")
