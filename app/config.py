...
class DatabaseConfig(BaseSettings):
    ...
    host: str = Field(default="10.0.20.6", env="DB_HOST")
...
class Settings(BaseSettings):
    ...
    def db_pool_size(self) -> int:
        """Get database pool size for backward compatibility."""
    ...
    def redis_url(self) -> str:
        ...
        password_part = f":{self.cache.password}@" if self.cache.password else ""
        return f"redis://{password_part}{self.cache.host}:{self.cache.port}/{self.cache.db}"
    
    @property
    def redis_max_connections(self) -> int:
        """Get Redis max connections for backward compatibility."""
        return self.cache.max_connections
